...
from text_embedders import rnn_embedder

slim = tf.contrib.slim


class RNNEmbedderTest(tf.test.TestCase):
  def setUp(self):
  ...
  def test_embed(self):
    ...
    with tf.Graph().as_default():
      ...
      self.assertIsInstance(embedder, rnn_embedder.RNNEmbedder)
    ...
    config = self.default_config
    config.rnn_embedder.vocab_size = 10001
    config.rnn_embedder.embedding_size = 199
    config.rnn_embedder.caption_embedding_size = 201
