...
flags.DEFINE_string('model_config', 
    'models/image_none.hard_32.hidden_x0.load_cap/model.pbtxt', 
    'Path to the configuration file.')

flags.DEFINE_string('saved_ckpts_dir',
    'log7/image_none.hard_32.hidden_x0.load_cap.tune_caption_true.adam_0.0001/saved_ckpts/', 
...
flags.DEFINE_integer('hcluster_num_clusters', 
    100, 'Number of clusters of hierachrchy clustering.')

flags.DEFINE_string('hcluster_model_path', 
    'output/hcluster.npz', 
    'Path to the hcluster model file.')
...
def _get_hcluster_model():
  # Process hcluster clustering and generate model file.
  if not os.path.isfile(FLAGS.hcluster_model_path):
    tf.logging.info('Training hcluster...')
    _, examples = _get_visual_features(is_training_example)
    examples_embeded = _embed_visual_features(examples[:10000])

    hcluster = AgglomerativeClustering(
        n_clusters=FLAGS.hcluster_num_clusters)

    hcluster.fit(np.concatenate(examples_embeded, axis=0))
    joblib.dump(hcluster, FLAGS.hcluster_model_path)

  # Load hcluster model.
  else:
    hcluster = joblib.load(FLAGS.hcluster_model_path)
    tf.logging.info('Load hcluster model from file %s.', FLAGS.hcluster_model_path)

  exit(0)
  return hcluster


def _get_visual_words(hcluster, filter_fn):
  if not os.path.isfile(FLAGS.visual_word_path):
    ...
    for image_id, example_embed in zip(image_ids, examples_embeded):
      visual_words[image_id] = hcluster.predict(example_embed).tolist()
...
def _show_visual_words(hcluster, examples):
  sim = WordSim()
  ...
  centers = hcluster.cluster_centers_
...
def main(_):
  ...
  hcluster = _get_hcluster_model()
  visual_words = _get_visual_words(hcluster, is_validation_example)

  examples = _index_examples(visual_words)
  tf.logging.info('Indexed %d examples.', len(examples))

  # Show visual words.
  _show_visual_words(hcluster, examples)
